...
export async function listOpsShareRows() {
  const result = await query<OpsShareRow>(
    `${SHARE_SELECT_FROM} ORDER BY ds.created_at DESC`
  );

  return result.rows;
}

export async function paginatedOpsShares(page: number, pageSize: number, search: string) {
  return paginatedQuery<OpsShareRow>(
    {
      selectFrom: SHARE_SELECT_FROM,
      searchColumns: [
        'COALESCE(d.serial_number, di.serial_number)',
        'd.name',
        'owner_user.short_uid',
        'owner_user.display_name',
        'target_user.short_uid',
        'target_user.display_name',
      ],
      filterColumns: {},
      orderBy: 'ORDER BY ds.created_at DESC',
    },
